...
@gin.configurable
# def vgg_like(input_shape, n_classes, base_filters, kernel_size, strides, n_blocks, dense_units, dropout_rate):
def vgg_like(input_shape, n_classes, base_filters, n_blocks, dense_units, dropout_rate):
    ...
    inputs = tf.keras.Input(input_shape)  # Input shape is (batch_size, 256, 256, 3)
    out = vgg_block(inputs, base_filters, kernel_size=(3, 3), strides=1)  # (batch_size, 256, 256, 3)->(batch_size, 128, 128, 32)
    for i in range(1, n_blocks):
        out = vgg_block(out, base_filters * (2 ** i), kernel_size=(3, 3), strides=1)  # (batch_size, 16, 16, 256)
    out = tf.keras.layers.GlobalAveragePooling2D()(out)  # (batch_size, 256)
