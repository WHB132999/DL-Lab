...
@gin.configurable
def vgg_block(inputs, filters, kernel_size, strides):
    ...
    out = tf.keras.layers.Conv2D(filters, kernel_size, strides=strides, padding='same', activation=tf.nn.relu)(inputs)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.Conv2D(filters, kernel_size, strides=strides, padding='same', activation=tf.nn.relu)(out)
    out = tf.keras.layers.BatchNormalization()(out)
...
# Just try to test which branch I will upload my files!!!
