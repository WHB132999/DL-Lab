...
import logging
import wandb
# from evaluation.Confusionmatrix import ConfusionMatrix
...
class Trainer(object):
    def __init__(self, model, ds_train, ds_val, ds_info, run_paths, total_steps, log_interval, ckpt_interval):
        ...
        self.restore = False  # CHANGE TO TRUE IF YOU WANT TO RESTORE FROM LATEST CHECKPOINT
        # Summary Writer
    ...
    def train_step(self, images, labels):
        with tf.GradientTape() as tape:
            ...
            loss_labels = labels - 1  # labels should be between 0 and 5 for loss, but are between 1 and 6
            loss = self.loss_object(loss_labels, predictions)
        gradients = tape.gradient(loss, self.model.trainable_variables)
        self.optimizer.apply_gradients(zip(gradients, self.model.trainable_variables))

        self.train_loss(loss)
        self.train_accuracy(loss_labels, predictions)
    ...
    def test_step(self, images, labels):
        ...
        loss_labels = labels - 1  # labels should be between 0 and 5 for loss, but are between 1 and 6
        t_loss = self.loss_object(loss_labels, predictions)

        self.test_loss(t_loss)
        self.test_accuracy(loss_labels, predictions)
    ...
    def train(self):
        ...
        for idx, (images, labels) in enumerate(self.ds_train):
            ...
            with self.train_summary_writer.as_default():  # (self.)train_summary..... ????
                tf.summary.scalar('loss', self.train_loss.result(), step=step)  # self.epoch)  # (self.)train_loss? ????
            ...
            if step % self.log_interval == 0:
                ...
                with self.test_summary_writer.as_default():
                    tf.summary.scalar('loss', self.test_loss.result(), step=step)  # self.epoch)
                    tf.summary.scalar('accuracy', self.test_accuracy.result(), step=step)  # self.epoch)

                template = 'Step {}, Loss: {}, Accuracy: {}, Test Loss: {}, Test Accuracy: {}'
                logging.info(template.format(step,
                                             self.train_loss.result(),  # missing ??!??!?!
            ...
            if step % self.ckpt_interval == 0:  # ckpt_interval defined before (e.g. 10)
